...
public class ProgramOrderService {
    ...
    public List<TicketCategoryVo> getTicketCategoryList(ProgramOrderCreateDto programOrderCreateDto, Date showTime) {
        ...
        List<TicketCategoryVo> ticketCategoryVoList = ticketCategoryService
                .selectTicketCategoryListByProgramIdMultipleCache(programOrderCreateDto.getProgramId(), showTime);
            ...
            for (SeatDto seatDto : seatDtoList) {
                ...
                if (Objects.nonNull(ticketCategoryVo)) {
                    ...
                    TicketCategoryVo matchByPrice = ticketCategoryVoList.stream()
                            .filter(tc -> Objects.equals(tc.getPrice(), seatDto.getPrice())).findFirst().orElse(null);
    ...
    public String create(ProgramOrderCreateDto programOrderCreateDto) {
        ...
        List<TicketCategoryVo> getTicketCategoryList = getTicketCategoryList(programOrderCreateDto,
        ...
        for (TicketCategoryVo ticketCategory : getTicketCategoryList) {
            List<SeatVo> allSeatVoList = seatService.selectSeatResolution(programOrderCreateDto.getProgramId(),
            ...
            ticketCategoryRemainNumber.putAll(ticketCategoryService
                    .getRedisRemainNumberResolution(programOrderCreateDto.getProgramId(), ticketCategory.getId()));
        }
        if (CollectionUtil.isNotEmpty(seatDtoList)) {
            ...
            for (Entry<Long, Long> entry : seatTicketCategoryDtoCount.entrySet()) {
                ...
                Long remainNumber = Optional
                        .ofNullable(ticketCategoryRemainNumber.get(String.valueOf(ticketCategoryId)))
            ...
            for (SeatDto seatDto : seatDtoList) {
                ...
                if (Objects.isNull(seatVo)) {
                    if (Objects.equals(programOrderCreateDto.getProgramId(), 2L)) {
                        ...
                        pseudo.setZoneName(seatDto.getZoneName());
            ...
            purchaseSeatList = SeatMatch.findAdjacentSeatVos(
                    seatVoList.stream().filter(seatVo -> Objects.equals(seatVo.getTicketCategoryId(), ticketCategoryId))
    ...
    public List<SeatVo> createOrderOperateProgramCacheResolution(ProgramOrderCreateDto programOrderCreateDto) {
        ...
        if (CollectionUtil.isNotEmpty(seatDtoList)) {
            ...
            for (SeatDto seatDto : seatDtoList) {
                if (Objects.isNull(seatDto.getId()) || seatDto.getId() <= 0) {
                    ...
                    Map<String, SeatVo> seatMap = ticketCategorySeatMap.computeIfAbsent(ticketCategoryId,
                            k -> seatService.getSeatVoListByCacheResolution(programId, k).stream()
                                    .filter(s -> Objects.equals(s.getSellStatus(), SellStatus.NO_SOLD.getCode()))
                                    .collect(Collectors.toMap(s -> s.getRowCode() + "-" + s.getColCode(), s -> s,
                                            (v1, v2) -> v2)));
        ...
        if (CollectionUtil.isNotEmpty(seatDtoList)) {
            ...
            for (Entry<Long, List<SeatDto>> entry : seatTicketCategoryDtoCount.entrySet()) {
                ...
                seatDatajsonObject.put("seatNoSoldHashKey",
                        RedisKeyBuild.createRedisKey(RedisKeyManage.PROGRAM_SEAT_NO_SOLD_RESOLUTION_HASH, programId,
                                ticketCategoryId).getRelKey());
            ...
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("programTicketRemainNumberHashKey",
                    RedisKeyBuild.createRedisKey(RedisKeyManage.PROGRAM_TICKET_REMAIN_NUMBER_HASH_RESOLUTION, programId,
                            ticketCategoryId).getRelKey());
            jsonObject.put("ticketCategoryId", ticketCategoryId);
            jsonObject.put("ticketCount", ticketCount);
            jsonObject.put("seatNoSoldHashKey", RedisKeyBuild
                    .createRedisKey(RedisKeyManage.PROGRAM_SEAT_NO_SOLD_RESOLUTION_HASH, programId, ticketCategoryId)
                    .getRelKey());
            jsonArray.add(jsonObject);
        }
        ...
        ProgramCacheCreateOrderData programCacheCreateOrderData = programCacheCreateOrderResolutionOperate
                .programCacheOperate(keys, data);
        if (!Objects.equals(programCacheCreateOrderData.getCode(), BaseCode.SUCCESS.getCode())) {
            throw new DaMaiFrameException(
                    Objects.requireNonNull(BaseCode.getRc(programCacheCreateOrderData.getCode())));
        }
    ...
    private OrderCreateDto buildCreateOrderParam(ProgramOrderCreateDto programOrderCreateDto,
        ...
        orderCreateDto
                .setOrderNumber(uidGenerator.getOrderNumber(programOrderCreateDto.getUserId(), ORDER_TABLE_COUNT));
        ...
        BigDecimal databaseOrderPrice = purchaseSeatList.stream().map(SeatVo::getPrice).reduce(BigDecimal.ZERO,
                BigDecimal::add);
        ...
        for (int i = 0; i < ticketUserIdList.size(); i++) {
            ...
            String zoneName = seatVo.getZoneName() != null ? seatVo.getZoneName() + " " : "";
            orderTicketUserCreateDto.setSeatInfo(zoneName + seatVo.getRowCode() + "排" + seatVo.getColCode() + "列");
    ...
    private void updateProgramCacheDataResolution(Long programId, List<SeatVo> seatVoList, OrderStatus orderStatus) {
        ...
        seatVoMap.forEach((k, v) -> {
            ...
            delSeatIdjsonObject.put("seatIdList",
